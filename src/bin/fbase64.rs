...
/// For encode/decode output, vmsplice references the output buffer pages
/// directly in the pipe (no kernel memcpy), saving ~1ms for 13.5MB output.
#[cfg(target_os = "linux")]
...
fn main() {
    ...
    // Saves ~1ms kernel memcpy for 13.5MB encode output through pipe.
    #[cfg(target_os = "linux")]
    let result = {
