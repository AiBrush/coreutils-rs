...
/// Stream buffer: 16MB — maximizes bytes per syscall for streaming paths.
/// The primary path uses splice+mmap (zero-copy), so this is the fallback
/// when splice is unavailable. 16MB means fewer read/write iterations:
/// for 10MB input, 1-2 iterations vs 3 with 4MB. The OS uses lazy allocation,
/// so only touched pages consume physical memory.
